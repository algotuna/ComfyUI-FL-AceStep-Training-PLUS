...
class FL_AceStep_Train:
    ...
    def train(self, model, config, tensor_dir, lora_name="my_lora", resume_from="", unique_id=None):
        ...
        try:
            ...
            # Gradient checkpointing (fork addition) — biggest activation-memory saver.
            # Tries the native HF hook first, then falls back to structure-agnostic
            # manual wrapping of the DiT transformer-block stack(s). See FORK_NOTES.md.
            if getattr(training_config, "gradient_checkpointing", False):
                self._enable_gradient_checkpointing(decoder, dit)
        ...
        optimizer = self._build_optimizer(trainable_param_list, training_config, device)
                ...
                for epoch in range(start_epoch, training_config.max_epochs):
                    ...
                    for batch in dataloader:
                        ...
                        if accumulation_step >= training_config.gradient_accumulation_steps:
                            ...
                            optimizer.zero_grad(set_to_none=True)
                            global_step += 1
                            self._maybe_empty_cache(training_config, global_step)
    ...
    def _build_optimizer(self, params, training_config, device):
        """Build the optimizer selected in the config (fork addition).

        Options:
          • AdamW (fused)      — torch AdamW with fused=True on CUDA (original default).
          • AdamW (standard)   — torch AdamW, no fused kernel (slightly less VRAM).
          • AdamW8bit          — bitsandbytes 8-bit optimizer state (~2x smaller).
          • PagedAdamW8bit     — 8-bit AND paged to CPU RAM (lowest VRAM).

        8-bit options require a bitsandbytes build for the GPU; if unavailable we
        fall back to fused/standard AdamW with a clear warning (no crash).
        """
        opt = getattr(training_config, "optimizer_type", "AdamW (fused)")
        lr = training_config.learning_rate
        wd = training_config.weight_decay

        if "8bit" in opt or "Paged" in opt:
            try:
                import bitsandbytes as bnb  # noqa: PLC0415
                cls = bnb.optim.PagedAdamW8bit if "Paged" in opt else bnb.optim.AdamW8bit
                where = "paged to CPU RAM" if "Paged" in opt else "in 8-bit on GPU"
                logger.info(f"Optimizer: bitsandbytes {cls.__name__} (state {where}).")
                return cls(params, lr=lr, weight_decay=wd)
            except Exception as e:
                logger.warning(
                    f"Requested '{opt}' but bitsandbytes is unavailable ({e}). "
                    "Falling back to AdamW. Install a bitsandbytes build matching your "
                    "GPU/CUDA to use 8-bit / CPU-paged optimizers."
                )

        kwargs = {"lr": lr, "weight_decay": wd}
        if opt == "AdamW (fused)" and device.type == "cuda":
            kwargs["fused"] = True
        logger.info(f"Optimizer: torch AdamW (fused={kwargs.get('fused', False)}).")
        return AdamW(params, **kwargs)

    @staticmethod
    def _maybe_empty_cache(training_config, global_step):
        """Periodically clear the CUDA cache to fight fragmentation (fork addition)."""
        n = getattr(training_config, "empty_cache_every_n_steps", 0)
        if n and global_step % n == 0 and torch.cuda.is_available():
            torch.cuda.empty_cache()

    def _enable_gradient_checkpointing(self, decoder, dit):
        """Enable gradient checkpointing (fork addition).

        Strategy:
          1. Make inputs require grad (PEFT helper) so checkpointing works through
             a frozen base + LoRA adapters.
          2. Try the native HF-style `gradient_checkpointing_enable()` hook.
          3. Fall back to structure-agnostic manual wrapping: discover the DiT's
             transformer-block ModuleList(s) and wrap each block's forward in
             `torch.utils.checkpoint` (use_reentrant=False).

        Everything is defensive — any failure logs a warning and continues without
        checkpointing rather than crashing the run.
        """
        if hasattr(decoder, "enable_input_require_grads"):
            try:
                decoder.enable_input_require_grads()
            except Exception as e:
                logger.warning(f"enable_input_require_grads() failed: {e}")

        # 1) Native hook, if the model provides one.
        for target in (decoder, getattr(decoder, "base_model", None), dit):
            if target is not None and hasattr(target, "gradient_checkpointing_enable"):
                try:
                    target.gradient_checkpointing_enable()
                    logger.info(f"Gradient checkpointing: native hook on {type(target).__name__}.")
                    return
                except Exception as e:
                    logger.warning(f"Native gradient_checkpointing_enable() failed on "
                                   f"{type(target).__name__}: {e}")

        # 2) Generic manual wrapping of the transformer-block stack(s).
        wrapped, stacks = self._wrap_transformer_blocks(decoder)
        if wrapped:
            logger.info(f"Gradient checkpointing: manually wrapped {wrapped} blocks "
                        f"across stack(s) {stacks}.")
        else:
            logger.warning("Gradient checkpointing requested but no transformer-block "
                           "ModuleList could be identified to wrap; continuing WITHOUT it. "
                           "Inspect the model and set the block stack manually if needed "
                           "(see FORK_NOTES.md).")

    @staticmethod
    def _wrap_transformer_blocks(root):
        """Wrap the forward() of each block in the DiT's transformer stack(s) with
        torch.utils.checkpoint. Returns (num_blocks_wrapped, [stack_names]).

        Heuristic: a transformer stack is a homogeneous `nn.ModuleList` (len >= 2)
        whose element type name contains "block" or "layer". Nested stacks (those
        living inside an already-chosen stack) are skipped to avoid double-wrapping.
        """
        import torch.utils.checkpoint as cp

        # Things a transformer "block"/"layer" name could collide with but isn't.
        _NOT_A_BLOCK = ("norm", "dropout", "embedding", "linear", "identity",
                        "act", "pool", "conv")

        def _is_block_stack(mod):
            type_names = {type(m).__name__ for m in mod}
            if len(type_names) != 1:
                return False
            tname = next(iter(type_names)).lower()
            if any(k in tname for k in _NOT_A_BLOCK):
                return False
            if "block" not in tname and "layer" not in tname:
                return False
            # A real transformer block has substantial parameters (attn + MLP);
            # this rules out stacks of trivial wrappers that slipped the name check.
            return sum(p.numel() for p in mod[0].parameters()) >= 1000

        candidates = []
        for name, module in root.named_modules():
            if isinstance(module, torch.nn.ModuleList) and len(module) >= 2 \
                    and _is_block_stack(module):
                candidates.append((len(module), name, module))

        # Drop stacks nested inside another candidate (compare by dotted-name prefix).
        candidates.sort(key=lambda x: x[0], reverse=True)
        chosen = []
        for length, name, module in candidates:
            if any(name == c or name.startswith(c + ".") or c.startswith(name + ".")
                   for _, c, _ in chosen):
                continue
            chosen.append((length, name, module))

        def _make_ckpt_forward(orig_forward):
            def _forward(*args, **kwargs):
                # Only checkpoint when building a graph (training); pass through at eval.
        ...
        total = 0
        names = []
        for length, name, module in chosen:
            for block in module:
                if getattr(block, "_fl_ckpt_wrapped", False):
                    continue
                try:
                    block.forward = _make_ckpt_forward(block.forward)
                    block._fl_ckpt_wrapped = True
                    total += 1
                except Exception as e:
                    logger.warning(f"Could not wrap a block in '{name}': {e}")
